...
func (t *ReplyRepository) CreateReply(input *models.ReplyInput, user *models.Profile) error {
	query := "INSERT INTO movie_reply (MovieID, UserID, UserName, UserSurname, UserAvatar, replyText) VALUES ($1, $2, $3, $4, $5, $6)"
	_, dbErr := t.dbConnection.Exec(query, input.MovieID, user.UserCredentials.ID, user.Name, user.Surname, user.AvatarPath, input.Text)
	if dbErr != nil {
...
func (t *ReplyRepository) GetMovieReplies(movieID, limit, offset int) (*[]models.ReplyModel, error) {
	query := "SELECT v1.ID, v1.MovieID, v1.UserID, v1.UserName, v1.UserSurname, v1.UserAvatar, v1.replyText, v2.movie_rating FROM movie_reply v1 " +
	...
	for rows.Next() {
		...
		scanErr := rows.Scan(&result.ID, &result.MovieID, &result.User.UserID, &result.User.Name, &result.User.Surname,
			&result.User.AvatarPath, &result.Text, &result.UserRating)
		if scanErr != nil {
			return nil, models.ErrFooInternalDBErr
		}
