...
func TestAuthenticationSuccessCases(t *testing.T){
	psqlInfo := fmt.Sprintf("host=%s port=%d user=%s password=%s dbname=%s sslmode=disable",
		Host, Port,User,Password,DBName)
	TestDBConnection, DBConnectionError := sql.Open("postgres", psqlInfo)
	...
	authRep := repository.NewAuthSQLRepository(TestDBConnection)
	authUseCase := usecase.NewUserUseCase(authRep)
}
